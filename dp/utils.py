...
def get_noise_multiplier(
    ...
    **kwargs,
    ...
    while eps_high > target_epsilon:
        ...
        eps_high, _ = accountant.get_epsilon(delta=target_delta, **kwargs)
        # print("sigma high : ", sigma_high, "eps_high : ", eps_high)
        if sigma_high > MAX_SIGMA:
            raise ValueError("The privacy budget is too low.")
        
    # print(target_epsilon, eps_high, epsilon_tolerance)

    while target_epsilon - eps_high > epsilon_tolerance:
        sigma = (sigma_low + sigma_high) / 2
        accountant.history = [(sigma, sample_rate, steps)]
        eps, _ = accountant.get_epsilon(delta=target_delta, **kwargs)
        # print(f"eps : {eps}, eps_high : {eps_high}, sigma_high : {sigma_high}, sigma_low : {sigma_low}")
